...
import { useActivateJdAnalysisPromptPair, useJdAnalysisPromptPair } from '@/hooks/use-prompts';
...
export function JdAnalysisPairActivationCard() {
  const { data: response, isLoading, isError } = useJdAnalysisPromptPair();
  const activateMutation = useActivateJdAnalysisPromptPair();
  const pair = response?.data;
  const [systemVersionId, setSystemVersionId] = useState('');
  const [userVersionId, setUserVersionId] = useState('');

  if (isLoading) {
    return <Card><CardContent className="py-6 text-sm text-muted-foreground">Loading JD analysis prompt pair...</CardContent></Card>;
  }

  if (isError || !pair) {
    return <Card><CardContent className="py-6 text-sm text-destructive">Failed to load the JD analysis prompt pair.</CardContent></Card>;
  }
  ...
  const handleActivate = () => {
    ...
    if (confirm('Activate these JD system and user prompt versions together? The previous pair will be replaced atomically.')) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Activate JD analysis prompt pair</CardTitle>
        <CardDescription>
          Choose one compatible system prompt and one user template. They are activated together, so a JD parse never uses a mixed pair.
        </CardDescription>
      </CardHeader>
