    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="overflow-hidden rounded-md border bg-background">
      <CollapsibleTrigger className="flex w-full items-center gap-3 p-3 text-left hover:bg-muted/40">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            ...
            {item.isCriticalGap ? <Badge variant="destructive" className="text-[10px] font-normal">{t("criticalGap")}</Badge> : null}
          </span>
          <span className="mt-2 flex gap-1" aria-label={t("scorePercentLabel", { score: Math.round(score * 100) })}>
            {[0.25, 0.5, 0.75, 1].map((step) => (
              <span key={step} className={`h-1.5 w-6 rounded-full ${score >= step ? barClass : "bg-muted"}`} />
            ))}
          </span>
        </span>
      ...
      <CollapsibleContent>
        <div className="space-y-3 border-t px-4 py-3">
          <p className="text-sm leading-relaxed text-muted-foreground">{item.reasoning || t("noReasoning")}</p>
