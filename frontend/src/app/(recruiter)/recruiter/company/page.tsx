...
export default function CompanyOverviewPage() {
  ...
  if (isLoading) {
    return <div className="py-12 text-center text-muted-foreground animate-pulse">{t('overviewLoading')}</div>;
  }
  ...
  if (!company) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center border-2 border-dashed rounded-xl bg-muted/10">
        ...
        <h2 className="text-2xl font-bold tracking-tight mb-2">{t('noCompanyTitle')}</h2>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardDescription className="text-base mt-1 flex items-center gap-2">
                  {company.industry} • {company.companyType && `${company.companyType} • `}{t('employees', { size: company.companySize })}
                </CardDescription>
              </div>
            </div>
          </div>
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-semibold text-muted-foreground mb-1">{t('headquarters')}</h4>
                <p>{company.headquartersAddress || t('notProvided')}</p>
              </div>
              <div>
                <h4 className="text-sm font-semibold text-muted-foreground mb-1">{t('website')}</h4>
                {company.website ? (
            ...
            <div className="bg-muted/30 rounded-lg p-4 border space-y-3">
              <h4 className="font-semibold border-b pb-2">{t('verificationStatus')}</h4>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {company.status === 'PENDING' && (
                    <p className="text-xs text-muted-foreground">{t('statusPendingInfo')}</p>
                  )}
                  {company.status === 'REJECTED' && (
                    <p className="text-xs text-red-500">{t('statusRejectedInfo')}</p>
                  )}
                  {company.status === 'VERIFIED' && (
                    <p className="text-xs text-green-600">{t('statusVerifiedInfo')}</p>
                  )}
                </div>
              </div>
