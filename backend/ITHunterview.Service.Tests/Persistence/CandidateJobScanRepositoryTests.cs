...
public sealed class CandidateJobScanRepositoryContractTests
{
    [Fact]
    ...
    public async Task CreatePendingAsync_SameScopeTwice_CreatesDistinctRuns_WithoutPostgres()
    {
        ...
        var repository = MatchingScanRepositoryFactory.Candidate(context);
        var candidateUserId = Guid.NewGuid();
        var cvId = Guid.NewGuid();

        var first = await repository.CreatePendingAsync(
            CreateRun(candidateUserId, cvId, At(0)),
        ...
        (await context.CandidateJobScanRuns.CountAsync()).Should().Be(2);
    }
        ...
        InvalidInitialLifecycle invalidLifecycle)
    {
        ...
        var candidateUserId = Guid.NewGuid();
        var cvId = Guid.NewGuid();
        var supplied = CreateRun(candidateUserId, cvId, At(0));
        ...
        var callerSnapshot = new CandidateJobScanRun
        {
            Id = supplied.Id,
            CandidateUserId = supplied.CandidateUserId,
            CvId = supplied.CvId,
            CvFileNameSnapshot = supplied.CvFileNameSnapshot,
        ...
        var persistedCount = await context.CandidateJobScanRuns.CountAsync(run => run.Id == supplied.Id);
    ...
    public async Task GetLatestCompletedAsync_UsesExactOwnerScopeAndCreatedOrder_WithoutPostgres()
    {
        ...
        var cvId = Guid.NewGuid();
        var olderCompleted = CreateRun(ownerId, cvId, At(0), MatchingScanRunStatus.Completed);
        var newerCompleted = CreateRun(ownerId, cvId, At(1), MatchingScanRunStatus.Completed);
        var newestFailed = CreateRun(ownerId, cvId, At(2), MatchingScanRunStatus.Failed);
        context.CandidateJobScanRuns.AddRange(olderCompleted, newerCompleted, newestFailed);
        await context.SaveChangesAsync();

        var latest = await repository.GetLatestCompletedAsync(ownerId, cvId, CancellationToken.None);
        var otherOwner = await repository.GetLatestCompletedAsync(Guid.NewGuid(), cvId, CancellationToken.None);
        var otherCv = await repository.GetLatestCompletedAsync(ownerId, Guid.NewGuid(), CancellationToken.None);

        latest!.Id.Should().Be(newerCompleted.Id);
        otherOwner.Should().BeNull();
        otherCv.Should().BeNull();
    }

    [Theory]
    [InlineData(MatchingScanRunStatus.Pending)]
    [InlineData(MatchingScanRunStatus.Processing)]
    [Trait("Requirement", "R-05")]
    [Trait("Requirement", "R-10")]
    public async Task GetPendingOrProcessingByIdAsync_ExactId_ReturnsFullStoredRunnableLifecycle_WithoutPostgres(
        MatchingScanRunStatus status)
    {
        await using var context = MatchingScanInMemoryContextFactory.Create();
        var repository = MatchingScanRepositoryFactory.Candidate(context);
        var candidateUserId = Guid.NewGuid();
        var cvId = Guid.NewGuid();
        var run = CreateRun(candidateUserId, cvId, At(0), status);
        run.StartedAt = status == MatchingScanRunStatus.Pending ? null : At(1);
        context.CandidateJobScanRuns.Add(run);
        await context.SaveChangesAsync();
        context.ChangeTracker.Clear();

        var owned = await repository.GetPendingOrProcessingByIdAsync(
            run.Id,
            CancellationToken.None);

        owned.Should().BeEquivalentTo(run);
        context.Entry(owned!).State.Should().Be(EntityState.Detached);
    }

    [Fact]
    [Trait("Requirement", "R-05")]
    [Trait("Requirement", "R-10")]
    public async Task GetPendingOrProcessingByIdAsync_MissingOrTerminalRun_ReturnsNull_WithoutPostgres()
    {
        await using var context = MatchingScanInMemoryContextFactory.Create();
        var repository = MatchingScanRepositoryFactory.Candidate(context);
        var candidateUserId = Guid.NewGuid();
        var cvId = Guid.NewGuid();
        var run = CreateRun(candidateUserId, cvId, At(0), MatchingScanRunStatus.Pending);
        var completed = CreateRun(candidateUserId, cvId, At(1), MatchingScanRunStatus.Completed);
        completed.StartedAt = At(2);
        completed.CompletedAt = At(3);
        var failed = CreateRun(candidateUserId, cvId, At(2), MatchingScanRunStatus.Failed);
        failed.StartedAt = At(3);
        failed.CompletedAt = At(4);
        failed.ErrorCode = "STORED_FAILURE";
        failed.ErrorMessage = "stored terminal failure metadata";
        context.CandidateJobScanRuns.AddRange(run, completed, failed);
        await context.SaveChangesAsync();
        context.ChangeTracker.Clear();

        var missingRun = await repository.GetPendingOrProcessingByIdAsync(
            Guid.NewGuid(),
            CancellationToken.None);
        var completedRun = await repository.GetPendingOrProcessingByIdAsync(
            completed.Id,
            CancellationToken.None);
        var failedRun = await repository.GetPendingOrProcessingByIdAsync(
            failed.Id,
            CancellationToken.None);

        missingRun.Should().BeNull();
        completedRun.Should().BeNull();
        failedRun.Should().BeNull();
    }

    private static CandidateJobScanRun CreateRun(
        ...
        MatchingScanRunStatus status = MatchingScanRunStatus.Pending) => new()
    {
        ...
        CvId = cvId,
        CvFileNameSnapshot = "synthetic.pdf",
        Status = status,
        CreatedAt = createdAt
    };

    private static DateTime At(int minute) =>
        new DateTime(2026, 8, 16, 1, 0, 0, DateTimeKind.Utc).AddMinutes(minute);
...
public sealed class CandidateJobScanRepositoryTests
{
    private readonly MatchingScanPostgresFixture _database;

    public CandidateJobScanRepositoryTests(MatchingScanPostgresFixture database)
    {
        _database = database;
    }
    ...
    public async Task CreatePendingAsync_SameScopeTwice_CreatesDistinctRuns()
    {
        ...
        first.Id.Should().NotBe(second.Id);
        (await context.CandidateJobScanRuns.CountAsync(run =>
                run.CandidateUserId == seed.CandidateUserId && run.CvId == seed.CvId))
            .Should().Be(2);
    }
    ...
    public async Task TryStartAsync_TwoConcurrentCallers_OnlyOneClaimsPendingRun()
    {
        ...
        {
            var createRepository = MatchingScanRepositoryFactory.Candidate(createContext);
            runId = (await createRepository.CreatePendingAsync(CreateRun(seed, At(0)), CancellationToken.None)).Id;
        }

        var updateBarrier = new MatchingScanUpdateBarrier("candidate_job_scan_runs");
        ...
        var firstRepository = MatchingScanRepositoryFactory.Candidate(firstContext);
        var secondRepository = MatchingScanRepositoryFactory.Candidate(secondContext);
        ...
        var persisted = await verifyContext.CandidateJobScanRuns.SingleAsync(run => run.Id == runId);
    ...
    public async Task CompleteAsync_WritesItemsAndTerminalStateAtomically()
    {
        await AssertSuccessfulCompletionAsync(
            Guid.Parse("60000000-0000-0000-0000-000000000031"),
            Guid.Parse("60000000-0000-0000-0000-000000000032"));
    }
    ...
    {
        await AssertSuccessfulCompletionAsync(
            Guid.Parse("60000000-0000-0000-0000-000000000041"),
            Guid.Parse("60000000-0000-0000-0000-000000000042"));
    }
    ...
    public async Task CompleteAsync_DuplicatePairInRun_RollsBackEntireCompletion()
    {
        ...
        CandidateJobScanRun beforeRun;
        await using (var beforeContext = _database.CreateContext())
        {
            beforeRun = await beforeContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
            (await beforeContext.CandidateJobScanResults.CountAsync(result => result.RunId == runId)).Should().Be(0);
        }
        ...
        {
            CreateResult(runId, seed.JobId, 1, Guid.Parse("60000000-0000-0000-0000-000000000011")),
            CreateResult(runId, seed.JobId, 2, Guid.Parse("60000000-0000-0000-0000-000000000012"))
        };
        ...
        await using var verifyContext = _database.CreateContext();
        var afterRun = await verifyContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
        afterRun.Should().BeEquivalentTo(beforeRun);
        (await verifyContext.CandidateJobScanResults.CountAsync(result => result.RunId == runId)).Should().Be(0);
    }
    ...
    public async Task Failure_DoesNotPublishPartialResultsOrLatestSnapshot()
    {
        ...
        {
            var repository = MatchingScanRepositoryFactory.Candidate(completedContext);
            await repository.CompleteAsync(
                completedRunId,
                [CreateResult(completedRunId, seed.JobId, 1)],
                At(1),
                CancellationToken.None);
        }
        ...
        {
            var repository = MatchingScanRepositoryFactory.Candidate(failingContext);
            var action = () => repository.CompleteAsync(
                failedCompletionRunId,
                [
                    CreateResult(failedCompletionRunId, seed.OtherJobId, 1),
                    CreateResult(failedCompletionRunId, seed.OtherJobId, 2)
                ],
                At(3),
                CancellationToken.None);
            await action.Should().ThrowAsync<Exception>();
        ...
        await using var verifyContext = _database.CreateContext();
        var verifyRepository = MatchingScanRepositoryFactory.Candidate(verifyContext);
        var latest = await verifyRepository.GetLatestCompletedAsync(
            seed.CandidateUserId,
            seed.CvId,
            CancellationToken.None);
        latest!.Id.Should().Be(completedRunId);
        (await verifyContext.CandidateJobScanResults.CountAsync(result => result.RunId == failedCompletionRunId))
            .Should().Be(0);
    }
    ...
    public async Task FailAsync_AfterCompleted_DoesNotMutateTerminalRun()
    {
        ...
        await repository.CompleteAsync(
            runId,
            [CreateResult(runId, seed.JobId, 1)],
            At(2),
            CancellationToken.None);

        CandidateJobScanRun beforeRun;
        IReadOnlyList<CandidateJobScanResult> beforeResults;
        await using (var beforeContext = _database.CreateContext())
        {
            beforeRun = await beforeContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
            beforeResults = await beforeContext.CandidateJobScanResults.AsNoTracking()
        ...
        await using var verifyContext = _database.CreateContext();
        var afterRun = await verifyContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
        var afterResults = await verifyContext.CandidateJobScanResults.AsNoTracking()
            .Where(result => result.RunId == runId)
            .OrderBy(result => result.Id)
            .ToListAsync();
        afterRun.Should().BeEquivalentTo(beforeRun);
    ...
    public async Task CompleteAsync_AfterFailed_DoesNotReopenRun()
    {
        ...
        await using (var context = _database.CreateContext())
        {
            ...
            beforeRun = await context.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
            var action = () => repository.CompleteAsync(
                runId,
                [CreateResult(runId, seed.JobId, 1)],
                At(2),
    ...
    public async Task GetLatestCompletedAsync_NewerFailed_ReturnsPreviousCompleted()
    {
        ...
        await using var context = _database.CreateContext();
        var currentRepository = MatchingScanRepositoryFactory.Candidate(context);
    ...
    public async Task GetLatestCompletedAsync_OlderFinishesLater_ReturnsNewerCreatedCompleted()
    {
        ...
        {
            var newerRepository = MatchingScanRepositoryFactory.Candidate(newerContext);
        ...
        {
            var olderRepository = MatchingScanRepositoryFactory.Candidate(olderContext);
        ...
        var readRepository = MatchingScanRepositoryFactory.Candidate(readContext);
    ...
    public async Task GetLatestCompletedAsync_OtherOwner_ReturnsNull()
    {
        ...
        (await repository.GetLatestCompletedAsync(Guid.NewGuid(), seed.CvId, CancellationToken.None))
            .Should().BeNull();
        (await repository.GetLatestCompletedAsync(seed.CandidateUserId, Guid.NewGuid(), CancellationToken.None))
            .Should().BeNull();
    }
    ...
    public async Task GetResultPageAsync_ReturnsOnlyRequestedRun_InRankThenIdOrder()
    {
        ...
        var rankOneLowerId = Guid.Parse("20000000-0000-0000-0000-000000000002");
        var rankOneHigherId = Guid.Parse("20000000-0000-0000-0000-000000000003");
        var rankTwoOpposingLowerId = Guid.Parse("20000000-0000-0000-0000-000000000001");
        await using var context = _database.CreateContext();
        var repository = MatchingScanRepositoryFactory.Candidate(context);
        await repository.CompleteAsync(
            requestedRunId,
            [
                CreateResult(requestedRunId, seed.OtherJobId, 1, rankOneHigherId),
                CreateResult(requestedRunId, seed.JobId, 1, rankOneLowerId),
                CreateResult(requestedRunId, seed.ThirdJobId, 2, rankTwoOpposingLowerId)
            ],
            At(2),
            CancellationToken.None);
        await repository.CompleteAsync(
            otherRunId,
            [CreateResult(otherRunId, seed.JobId, 0)],
            At(3),
            CancellationToken.None);
    ...
    public async Task TryStartAsync_CompletedOrFailed_ReturnsFalseWithoutMutatingTerminalRuns()
    {
        ...
        var completedResult = CreateResult(
            ...
            seed.JobId,
            1,
            Guid.Parse("60000000-0000-0000-0000-000000000021"));
        Guid failedRunId;
        await using (var setupContext = _database.CreateContext())
        {
            var setupRepository = MatchingScanRepositoryFactory.Candidate(setupContext);
        ...
        CandidateJobScanRun beforeCompletedRun;
        CandidateJobScanRun beforeFailedRun;
        IReadOnlyList<CandidateJobScanResult> beforeCompletedResults;
        IReadOnlyList<CandidateJobScanResult> beforeFailedResults;
        await using (var beforeContext = _database.CreateContext())
        {
            beforeCompletedRun = await beforeContext.CandidateJobScanRuns.AsNoTracking()
                .SingleAsync(run => run.Id == completedRunId);
            beforeFailedRun = await beforeContext.CandidateJobScanRuns.AsNoTracking()
                .SingleAsync(run => run.Id == failedRunId);
            beforeCompletedResults = await beforeContext.CandidateJobScanResults.AsNoTracking()
                .Where(result => result.RunId == completedRunId)
                .OrderBy(result => result.Id)
                .ToListAsync();
            beforeFailedResults = await beforeContext.CandidateJobScanResults.AsNoTracking()
        ...
        {
            var attemptRepository = MatchingScanRepositoryFactory.Candidate(attemptContext);
        ...
        await using var verifyContext = _database.CreateContext();
        var afterCompletedRun = await verifyContext.CandidateJobScanRuns.AsNoTracking()
            .SingleAsync(run => run.Id == completedRunId);
        var afterFailedRun = await verifyContext.CandidateJobScanRuns.AsNoTracking()
            .SingleAsync(run => run.Id == failedRunId);
        var afterCompletedResults = await verifyContext.CandidateJobScanResults.AsNoTracking()
            .Where(result => result.RunId == completedRunId)
            .OrderBy(result => result.Id)
            .ToListAsync();
        var afterFailedResults = await verifyContext.CandidateJobScanResults.AsNoTracking()
            .Where(result => result.RunId == failedRunId)
            .OrderBy(result => result.Id)
            .ToListAsync();
    ...
    public async Task CompleteAsync_Pending_RejectsWithoutResults()
    {
        ...
        var beforeRun = await context.CandidateJobScanRuns.AsNoTracking().SingleAsync(value => value.Id == run.Id);

        var action = () => repository.CompleteAsync(
            run.Id,
            [CreateResult(run.Id, seed.JobId, 1)],
            At(1),
            CancellationToken.None);

        await action.Should().ThrowAsync<Exception>();
        await using var verifyContext = _database.CreateContext();
        var afterRun = await verifyContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(value => value.Id == run.Id);
        afterRun.Should().BeEquivalentTo(beforeRun);
        (await verifyContext.CandidateJobScanResults.CountAsync(result => result.RunId == run.Id)).Should().Be(0);
    }
    ...
    public async Task FailAsync_PendingOrProcessing_TransitionsBothToFailed()
    {
        ...
        await using var verifyContext = _database.CreateContext();
        var failed = await verifyContext.CandidateJobScanRuns
    ...
    public async Task FailAsync_AfterFailed_DoesNotMutateTerminalRun()
    {
        ...
        await using (var beforeContext = _database.CreateContext())
        {
            beforeRun = await beforeContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(value => value.Id == run.Id);
        }
    ...
    public async Task CompleteAsync_AfterCompleted_DoesNotDuplicateResults()
    {
        ...
        var action = () => repository.CompleteAsync(
            runId,
            [CreateResult(runId, seed.OtherJobId, 2)],
            At(2),
            CancellationToken.None);
    ...
    public async Task FailAsync_LongOperationalMetadata_PersistsBoundedValues()
    {
        ...
        await using var verifyContext = _database.CreateContext();
        var persisted = await verifyContext.CandidateJobScanRuns.SingleAsync(value => value.Id == run.Id);
    ...
    public async Task ConcurrentCompleteAndFail_ProducesOneImmutableTerminalState()
    {
        ...
        var completeRepository = MatchingScanRepositoryFactory.Candidate(completeContext);
        var failRepository = MatchingScanRepositoryFactory.Candidate(failContext);
        ...
        await using var verifyContext = _database.CreateContext();
        var terminalRun = await verifyContext.CandidateJobScanRuns.AsNoTracking().SingleAsync(run => run.Id == runId);
        var terminalResults = await verifyContext.CandidateJobScanResults.AsNoTracking()
            .Where(result => result.RunId == runId)
            .OrderBy(result => result.Id)
            .ToListAsync();
        ...
        {
            var lateFailRepository = MatchingScanRepositoryFactory.Candidate(lateFailContext);
        ...
        {
            var lateCompleteRepository = MatchingScanRepositoryFactory.Candidate(lateCompleteContext);
    ...
    private async Task AssertSuccessfulCompletionAsync(Guid firstResultId, Guid secondResultId)
    {
        ...
        var results = new[]
        {
            CreateResult(runId, seed.JobId, 1, firstResultId),
            CreateResult(runId, seed.OtherJobId, 2, secondResultId)
        };
        ...
        var persistedResults = await verifyContext.CandidateJobScanResults.AsNoTracking()
                .Where(result => result.RunId == runId)
    ...
    private static CandidateJobScanResult CreateResult(
        Guid runId,
        Guid jobId,
        int rank,
        Guid? id = null) => new()
    {
        ...
        RunId = runId,
        JobId = jobId,
        JobTitleSnapshot = $"Synthetic job {rank}",
        MatchScore = 75m - rank,
        MatchDetails = $"synthetic candidate match details {rank}",
    ...
    private static DateTime At(int minute) =>
        new DateTime(2026, 8, 16, 2, 0, 0, DateTimeKind.Utc).AddMinutes(minute);
}
