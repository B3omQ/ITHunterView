...
public sealed class RecruiterCvScanRepository : IRecruiterCvScanRepository
{
    ...
    public RecruiterCvScanRepository(ITHunterviewContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        _context = context;
    }

    public async Task<RecruiterCvScanRun> CreatePendingAsync(
        RecruiterCvScanRun run,
        CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(run);
        EnsureCleanPendingRun(run);

        await _context.RecruiterCvScanRuns.AddAsync(run, ct);
    ...
    {
        var affected = await _context.RecruiterCvScanRuns
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (affected != 1)
            {
                throw new InvalidOperationException("Only a processing recruiter scan run can be completed.");
            }

            _context.RecruiterCvScanResults.AddRange(results);
            await _context.SaveChangesAsync(ct);
        ...
        CancellationToken ct)
    {
        ...
        await _context.RecruiterCvScanRuns
            .Where(run =>
                run.Id == runId &&
    ...
    public Task<RecruiterCvScanRun?> GetLatestCompletedAsync(
        Guid recruiterUserId,
        Guid companyId,
        Guid jobId,
        CancellationToken ct) =>
        _context.RecruiterCvScanRuns
            .AsNoTracking()
            .Where(run =>
                run.RecruiterUserId == recruiterUserId &&
    ...
    public async Task<(IReadOnlyList<RecruiterCvScanResult> Items, int TotalCount)> GetResultPageAsync(
        Guid runId,
        int skip,
        int take,
        CancellationToken ct)
    {
        var query = _context.RecruiterCvScanResults
            .AsNoTracking()
    ...
    public async Task<(RecruiterCvScanResult Result, RecruiterCvScanRun Run)?> GetOwnedResultAsync(
        Guid scanResultId,
        Guid recruiterUserId,
        CancellationToken ct)
    {
        var owned = await (
                from result in _context.RecruiterCvScanResults.AsNoTracking()
                join run in _context.RecruiterCvScanRuns.AsNoTracking()
                    on result.RunId equals run.Id
                where result.Id == scanResultId && run.RecruiterUserId == recruiterUserId
                select new { Result = result, Run = run })
            .SingleOrDefaultAsync(ct);

        return owned is null ? null : (owned.Result, owned.Run);
    }
    ...
    private static void EnsureResultsBelongToRun(
        Guid runId,
        IEnumerable<RecruiterCvScanResult> results)
    {
        if (results.Any(result => result.RunId != runId))
        {
            throw new ArgumentException("Every recruiter scan result must belong to the completed run.", nameof(results));
        }
    }

    private static void EnsureCleanPendingRun(RecruiterCvScanRun run)
    {
            ...
            run.ErrorMessage is not null)
        {
            throw new ArgumentException(
                "A new recruiter scan run must have a clean Pending lifecycle state.",
                nameof(run));
        }
    }
